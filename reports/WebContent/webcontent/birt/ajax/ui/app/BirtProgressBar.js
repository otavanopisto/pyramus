...
BirtProgressBar.prototype = Object.extend( new AbstractUIComponent( ),
{
	...
	__l_show : function( )
	{
		 ...
		 * 
		 * Changed progress bar's position to top center because in report 
		 * is embedded into Pyramus by iframe and thus the progress bar
		 * would be in incorrect position when centered.
		 */
		var scLeft = parseInt( document.documentElement.scrollLeft, 10);
		var x = ( scLeft + ( ( BirtPosition.viewportWidth( ) - this.__instance.offsetWidth ) / 2 ) );
		BirtPosition.move(this.__instance, x, 10);

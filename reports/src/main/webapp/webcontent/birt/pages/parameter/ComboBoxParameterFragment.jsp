...
<TR>
	...
	<TD nowrap="nowrap">
		<FONT><LABEL FOR="<%= encodedParameterName + "_selection"%>"><%= parameterBean.getDisplayName( ) %>:</LABEL></FONT>
...
%>
			<OPTION VALUE="<%= ParameterAccessor.htmlEncode( defaultValue ) %>" 
				...
				<%=  flag ? "SELECTED=\"SELECTED\"" : "" %> > <%= ParameterAccessor.htmlEncode( defaultDisplayText ) %></OPTION>
<%	
		}
...
%>
			<OPTION VALUE="<%= outputValue %>"
			        TITLE="<%= outputLabel %>"
			        selected="selected"><%= outputLabel %></OPTION>
<%

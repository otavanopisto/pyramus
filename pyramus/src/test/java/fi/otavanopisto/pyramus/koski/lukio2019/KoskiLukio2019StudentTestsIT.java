...
public class KoskiLukio2019StudentTestsIT extends AbstractRESTServiceTest {
  ...
  @Test
  public void testDoubleAssessment() {
    Person person = tools().createPerson();
    Student student = tools().createStudent(person.getId(), 1l);
    Course course = tools().createCourse(getClass().getSimpleName(), ORGANIZATIONID, 4l, 1);
    CourseStudent courseStudent = tools().createCourseStudent(course.getId(), student.getId());
    CourseAssessment courseAssessment = tools().createCourseAssessment(course.getId(), student.getId(), courseStudent.getId(), 7l, OffsetDateTime.now().minusDays(1));
    CourseAssessment courseAssessment2 = tools().createCourseAssessment(course.getId(), student.getId(), courseStudent.getId(), 8l, OffsetDateTime.now());
    try {
      Response response = given().headers(getAuthHeaders())
        .get("/persons/persons/{ID}/oppija", person.getId());
  ...
  public void testDoubleAssessmentSorting() {
    OffsetDateTime newerAssessmentDate = OffsetDateTime.now();
    OffsetDateTime olderAssessmentDate = OffsetDateTime.now().minusDays(1);

...
<html>
  <head>
    ...
    <script type="text/javascript">
      ...
      function openSearchUsersDialog(courseModuleId) {
        ...
        dialog.addDialogListener(function(event) {
          ...
          switch (event.name) {
            case 'okClick':
              $("assessingUserId." + courseModuleId).value = event.results.user.id;
              $("assessingUserName." + courseModuleId).innerHTML = event.results.user.name;
            break;
          }
        });
        dialog.open();
      ...
      <div class="genericFormContainer"> 
        ...
        <form action="savecourseassessment.json" id="courseAssessmentForm" method="post" ix:jsonform="true" ix:useglasspane="true">
          ...
          <div class="tabLabelsContainer" id="tabs">
            <c:forEach var="courseModule" items="${courseStudent.course.courseModules}">
              <a class="tabLabel" href="#cm-${courseModule.id}">${courseModule.subject.name} ${courseModule.courseNumber}</a>
            </c:forEach>
          </div>

          <c:forEach var="courseModule" items="${courseStudent.course.courseModules}">
            <c:set var="assessment" value="${assessments[courseModule.id]}" />
            ...
            <div id="cm-${courseModule.id}" class="tabContent">    
              ...
              <div class="genericFormSection">
                ...
                <input type="hidden" id="assessingUserId.${courseModule.id}" name="assessingUserId.${courseModule.id}" value="${assessingUserId}"/>
                <span id="assessingUserName.${courseModule.id}">${assessingUserName}</span>
              ...
              <div class="genericFormSection">  
                ...
                <select name="gradeId.${courseModule.id}">
                  <option value=""></option>
              ...
              <div class="genericFormSection">  
                ...
                <textarea name="verbalAssessment.${courseModule.id}" cols="60" rows="6" ix:cktoolbar="courseGradeText" ix:ckeditor="true">${verbalAssessment}</textarea>
              </div>
            </div>
          </c:forEach>
